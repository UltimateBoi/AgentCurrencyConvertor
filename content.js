async function waitForSelectorWithText(selector) {
    return new Promise((resolve) => {
        const poller = () => {
            ...
            if (element && element.tagName === 'STRONG' && element.textContent.trim()) {
...
async function convertCurrency(observer, goodsTxtSelector) {
    ...
    const amountCNYText = goodsTxtElement.textContent.match(/(\d+(\.\d+)?)/);
    ...
    if (!isNaN(amountCNY)) {
        try {
            ...
            if (goodsRmbElement) {
                if (window.location.hostname === 'www.superbuy.com') {
